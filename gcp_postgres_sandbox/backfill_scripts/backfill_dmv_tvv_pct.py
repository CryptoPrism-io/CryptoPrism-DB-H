...
# Engine for AI database (cp_ai) - for hourly OHLCV data AND crypto listings
...
# Load crypto listings from cp_ai database (get unique slugs from OHLCV data itself)
with engine_cpai.connect() as connection:
    # Get unique coin identifiers from the OHLCV data directly
    query = "SELECT DISTINCT id, slug, name, symbol FROM ohlcv_1h_250_coins ORDER BY slug"
...
# @title BACKFILL MODE: Keep ALL Timestamps (Not Just Latest)
# MODIFIED FOR BACKFILL: Process all historical timestamps, not just latest
# Original line removed: pct_change = df.loc[df.groupby('slug')['timestamp'].idxmax()]
pct_change = df.copy()  # Keep ALL timestamps for historical backfill
